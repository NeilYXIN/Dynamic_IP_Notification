...
def initialize_gmail_api():
    ...
    Perform authentication for first-time usage.
    """
    creds = None
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # creds = flow.run_local_server(port=0) # Uncomment to authenticate in a browser, for servers with GUI
            creds = flow.run_console() # Launch authentication in the console, for headless servers
...
def cache_exist(cache_path):
    """Detect if cache file already exists.

    Args:
      cache_path: Path to the cache file.

    Returns:
      True if cache file exists, otherwise False.
    """
...
def send_gmail(service, sender_email, receiver_email, msg_subject, curr_ip):
    """A wrapper for create_message() and send_message().
    """
    msg_text = "\nIP updated:\n" + \
        curr_ip  # Compose the email message here, begin with '\n' to separate message and header.
    message = create_message(
        sender_email, receiver_email, msg_subject, msg_text)
    send_message(service, sender_email, message)


def write_cache(cache_path, ip):
    """Write current IP into the cache file.
...
if __name__ == "__main__":
    ...
    if not cache_exist(cache_path):
        ...
        if cached_ip != curr_ip:
            # Update cache file
            write_cache(cache_path, curr_ip)
            print("[IP Updater] IP change detected: " +
                  cached_ip + " -> " + curr_ip)
            # Send email notification
            send_gmail(service, sender_email,
                       receiver_email, msg_subject, curr_ip)
        else:
